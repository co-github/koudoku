module Koudoku
  class SubscriptionsController < ApplicationController
    ...
    def load_owner
      unless params[:owner_id].nil?
        if current_owner.present?
          ...
          # below.)
          searched_owner = begin
                             current_owner.class.find(params[:owner_id])
                           rescue
                             nil
                           end
          ...
          if searched_owner.nil? && current_owner.class.respond_to?(:friendly)
            searched_owner = begin
                               current_owner.class.friendly.find(params[:owner_id])
                             rescue
                               nil
                             end
          end
    ...
    def load_subscription
      ...
      @subscription.present? ? @subscription : unauthorized
    end
    ...
    def index
      ...
      if current_owner && current_owner.subscription.present?
    ...
    def create
      ...
      flash[:error] = 'Unfortunately, your payment information was invalid. Please try again.'
      redirect_to koudoku.new_owner_subscription_path(current_user)
    end
    ...
    def subscription_params
      ...
      if defined?(ActionController::StrongParameters)
        params.require(:subscription).permit(:plan_id, :stripe_id, :current_price, :credit_card_token, :card_type, :last_four, :quantity)
      else
